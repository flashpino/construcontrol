...
import { Lightbulb, FileText, Calendar, User, MapPin, Edit, Trash2, X, Save, CheckCircle2, Clock, AlertCircle, XCircle } from 'lucide-react';
...
function StatusBadge({ value }) {
  ...
  if (!opt) return <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">—</span>;
...
export default function AcoesComplementares({ registros_com_acoes }) {
  ...
  const startEditing = (registro) => {
    ...
    editForm.setData({
      status_acao_complementar: registro.status_acao_complementar || '',
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex gap-3 flex-wrap">
              {STATUS_OPTIONS.map(({ value, label, color, Icon }) => (
                <span key={value} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-widest ${color}`}>
                  <Icon size={10} /> {label}
                </span>
              ))}
            </div>
            ...
            <CardContent className="p-0">
              {lista.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                  ...
                  <p className="font-bold uppercase tracking-widest text-xs">Nenhum registro com ação complementar</p>
                  <p className="text-sm mt-1 text-slate-500 font-medium">As ações aparecerão aqui conforme forem registradas no RDO.</p>
                </div>
              ) : (
                              ...
                              <div className="flex flex-wrap gap-2">
                                {STATUS_OPTIONS.map(({ value, label, color, Icon }) => (
                                  <button
                                    ...
                                    className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${editForm.data.status_acao_complementar === value ? color + ' ring-2 ring-offset-1 ring-slate-400' : 'bg-white border-slate-200 text-slate-400 hover:border-slate-300'}`}
                                  >
                                    <Icon size={11} /> {label}
                                  </button>
                                ))}
